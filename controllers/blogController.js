...
async function getBlog(req, res, next) {
    try {
        ...
        const resData = data.map((blog) => { return { id: blog._id, title: blog.Title, blogContent: blog.BlogContent } });
...
async function getBlogById(req, res, next) {
    try {
        ...
        res.status(200).json({ success: true, msg: "Successfully retrieved blog.", data: { id: data._id, title: data.Title, blogContent: data.BlogContent } });
    } catch (err) {
        console.log(err);
...
async function postBlog(req, res, next) {
    try {
        ...
        const data = await Blog.create({ Title: title, BlogContent: blogContent });
